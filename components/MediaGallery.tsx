    ...
    <div className="flex flex-col h-full bg-background">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2">
            {mediaData.images.length === 0 ? (
              ...
              mediaData.images.map((item, index) => (
                <div
                  key={index}
                  className="group relative aspect-square bg-surface rounded-xl overflow-hidden cursor-pointer transition-all hover:scale-[1.02] hover:shadow-lg hover:shadow-glow-purple/20"
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                    <div className="absolute bottom-0 left-0 right-0 p-3">
                      <div className="text-xs text-white font-medium truncate mb-0.5">{item.attachment.name}</div>
                      <div className="text-[10px] text-white/60">{formatTimestamp(item.message.timestamp)}</div>
                    </div>
                  </div>
                  {/* Hover indicator */}
      ...
      {previewAttachment && isImageFile(previewAttachment.type) && (
        ...
        >
          ...
          <button
            onClick={() => setPreviewAttachment(null)}
            className="absolute top-6 right-6 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm flex items-center justify-center text-white transition-all hover:scale-110 z-10"
          >
          ...
          >
            <img
              ...
              className="max-w-full max-h-[90vh] object-contain transition-transform duration-200 rounded-lg shadow-2xl"
              style={{ transform: `scale(${previewZoom})` }}
            />
            
            {/* Info Bar */}
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-black/80 backdrop-blur-md px-6 py-3 rounded-full border border-white/10">
              <div className="flex items-center space-x-4 text-white">
                <span className="text-sm font-medium truncate max-w-xs">{previewAttachment.name}</span>
                <div className="w-px h-4 bg-white/20"></div>
                <div className="flex items-center space-x-2">
                  <svg className="w-4 h-4 text-white/60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7" />
                  </svg>
                  <span className="text-xs text-white/80 font-mono">{Math.round(previewZoom * 100)}%</span>
                </div>
              </div>
            </div>
            ...
            {/* Zoom Hint */}
            {previewZoom === 1 && (
              <div className="absolute top-6 left-1/2 -translate-x-1/2 bg-black/60 backdrop-blur-sm px-4 py-2 rounded-full text-white/60 text-xs">
                Mausrad zum Zoomen
              </div>
            )}
          </div>
